...
const WorkCard = (props: Props) => {
  ...
  // Card variant (compact design for home page)
  return (
    <article className="group border rounded-lg bg-card hover:border-primary/20 hover:shadow-sm transition-all duration-200 p-4">
      <div className="flex items-start gap-3">
        {/* Company Logo */}
        <div className="shrink-0">
          {data.companyUrl ? (
            <Link href={data.companyUrl} target="_blank" className="block">
              <div className="w-10 h-10 rounded-lg bg-secondary flex items-center justify-center border border-border hover:bg-secondary/80 transition-colors">
                {data.logo && !imageError ? (
                  <Image
                    src={getImagePath(data.logo.url)}
                    alt={data.logo.alt || data.company}
                    width={data.logo.width || 24}
                    height={data.logo.height || 24}
                    className="object-contain w-6 h-6"
                    unoptimized
                    onError={() => setImageError(true)}
                  />
                ) : (
                  <span className="text-sm font-semibold text-foreground">{companyInitial}</span>
                )}
              </div>
            </Link>
          ) : (
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-2 mb-1">
            <div className="flex-1 min-w-0">
              <h3 className="text-base font-semibold font-heading mb-0.5 group-hover:text-primary transition-colors">
                {data.companyUrl ? (
                  <Link href={data.companyUrl ?? "#"} target="_blank" className="hover:text-primary transition-colors">
                    {data.company}
                  </Link>
              ...
              <h4 className="text-sm font-medium text-muted-foreground font-heading">
                {data.position}
              </h4>
            </div>
          </div>
